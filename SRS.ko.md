...
7. **컨텍스트 길이 제어:** `BONSAI_CTX`(기본 8192)로 KV 캐시 크기를 제한해 불필요한 VRAM 낭비를 방지. `config.bat`에서 값 조절 가능.
...
| **REQ-009** | 리소스 관리 | 컨텍스트 길이 제한 | `BONSAI_CTX`(기본 8192)를 `start_llama_server.ps1` 호출에 `-c` 추가 인자로 넘겨, 저장소 스크립트가 기본으로 할당하는 262144 토큰짜리 KV 캐시를 줄임(문제 7 참조). `config.bat`에 저장되어 사용자가 직접 조절 가능. | **Medium** |
...
- **실측 생성 속도(참고용):** RTX 5070 Ti(16GB) 기준, 27B 비전 응답 시 ternary(2-bit, Q2_0) 약 48~50 t/s, bonsai(1-bit, Q1_0) 약 54~59 t/s. `BONSAI_CTX=8192`, `-ngl 99`(전 레이어 GPU) 조건에서 측정.
