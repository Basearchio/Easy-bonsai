...
7. **Context length control:** `BONSAI_CTX` (default 8192) caps the KV cache size to avoid wasting VRAM. Adjustable via `config.bat`.
...
| **REQ-009** | Resource management | Context length cap | Passes `BONSAI_CTX` (default 8192) as an extra `-c` argument to the `start_llama_server.ps1` call, shrinking the 262144-token KV cache the repo script allocates by default (see Problem 7). Stored in `config.bat` so the user can adjust it directly. | **Medium** |
...
- **Measured generation speed (for reference):** On an RTX 5070 Ti (16GB), for a 27B vision response: ternary (2-bit, Q2_0) ~48-50 t/s, bonsai (1-bit, Q1_0) ~54-59 t/s. Measured with `BONSAI_CTX=8192` and `-ngl 99` (all layers on GPU).
